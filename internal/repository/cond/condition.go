...
func Parse(v any) Condition {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		default:
			log.Warn("unsupported field type: %v.", f.Kind())
			continue
		}
	}
	...
	cond := new(condImpl)
	cond.exp = strings.TrimSuffix(cs.String(), str.Space+str.And+str.Space)
	cond.args = vs
	log.Debug("parsed condition exp: %s, args: %v", cond.exp, cond.args)
	return cond
}

func inSql(fn string) string {
	sb := strings.Builder{}
	sb.WriteString(str.CamelCaseToUnderscore(fn))
	sb.WriteString(str.Space)
	sb.WriteString(str.In)
	sb.WriteString(str.Space)
	sb.WriteString(str.LeftBracket)
	sb.WriteString(str.QuestionMark)
	sb.WriteString(str.RightBracket)
	sb.WriteString(str.Space)
