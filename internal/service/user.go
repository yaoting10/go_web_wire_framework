...
func NewUserService(service *Service, userRepo *repository.UserRepository, conf config.Conf, s3c *s3.Client, rc redisx.Client,
	sdsrv *SysDeviceService, sssrv *SysSettingService, imgsrv *ImageService) *UserService {
	return &UserService{
		Service: service,
		s3c:     s3c,
		conf:    conf,
...
func (srv *UserService) Register(u *model.User, inviteCode string) bool {
	...
	err := srv.urpo.W.Transaction(func(tx *gorm.DB) error {
		errs.Throw(srv.makesureDailyInviteCodeNotUsed(tx, inviteCode))
		errs.Throw(tx.Create(&u).Error)
		// act := &model.Account{UserId: u.ID}
		// errs.Throw(tx.Create(act).Error)
		// 设置父级数据
		if u.ParentId > 0 {
...
func (srv *UserService) UpdateUser(request request.UpdateUserReq) {
	...
	if request.Telegram != "" {
		dbuser.Telegram = &request.Telegram
	}
	if request.Discord != "" {
		dbuser.Discord = &request.Discord
	}
...
func (srv *UserService) CompressImage(file io.Reader, filename string, typ int) (r io.Reader) {
	...
	case ".png":
		...
		errs.Throw(err)
		img = util.Thumbnail(w, w, img)
		var buf bytes.Buffer
		errs.Throw(png.Encode(&buf, img))
	...
	case ".gif":
		...
		img = util.Thumbnail(w, w, img)
		var buf bytes.Buffer
		errs.Throw(gif.Encode(&buf, img, nil))
		r = bytes.NewReader(buf.Bytes())
	case ".bmp":
	...
	case ".jpg", ".jpeg": // jpg
		...
		img = util.Thumbnail(w, w, img)
		var buf bytes.Buffer
		errs.Throw(jpeg.Encode(&buf, img, nil))
		r = bytes.NewReader(buf.Bytes())
	default:
		r = srcr // 不支持压缩
	}
...
func (srv *UserService) CheckValidEmail(email string, checkDomain bool) bool {
	if !mail.ValidMailAddress(email) {
		return false
	}
	if !checkDomain {
		return true
...
func (srv *UserService) FindInternalUsers() []*model.User {
	var us []*model.User
	errs.Throw(srv.urpo.R.Model(model.User{}).Where("internal = ? and status = ?", true, model.UserStatusVerified).Find(&us).Error)
	return us
}
...
func (srv *UserService) FindVipUsers() []*model.User {
	var us []*model.User
	errs.Throw(srv.urpo.R.Model(model.User{}).Where("vip = ? and status = ?", true, model.UserStatusVerified).Find(&us).Error)
	return us
}
