...
func mockToken(userId uint) string {
	...
	middle := ciphers.MD5(util.UUID32())
...
func TestEnc(t *testing.T) {
	...
	fmt.Println(key)
	iv := []byte(key)                                                                     // 随机iv，每次的密文不同
	rawBytes := errs.Throwv(ciphers.AES.Encrypt([]byte(s), []byte(key), ciphers.CBC, iv)) // aes解密
	encstr := fmt.Sprintf("%x\n", rawBytes)
...
func TestDec(t *testing.T) {
	...
	fmt.Println(key)
	iv := []byte(key)                                                                    // 随机iv，每次的密文不同
	encBytes := errs.Throwv(hex.DecodeString(s))                                         // 十六进制解码
	rawBytes := errs.Throwv(ciphers.AES.Decrypt(encBytes, []byte(key), ciphers.CBC, iv)) // aes解密
