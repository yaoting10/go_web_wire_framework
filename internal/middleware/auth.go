...
import (
	...
	"github.com/hankmor/gotools/str"
)
...
func AuthMiddleware(logger *log.Logger, conf config.Conf, redis redisx.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if m != http.MethodGet { // 非get请求需要防重复提交
			...
			var key string
			if userId == FixTokenUserId {
				key = redisLockPrefix + str.String(ip)
			} else {
				key = redisLockPrefix + str.String(userId)
			}
...
func getCryptoParam(c *gin.Context, logger *log.Logger) string {
	var crypto []string
...
func decryptAndRebind(ctx *gin.Context, crypto string, req *http.Request, userId int64, token string, logger *log.Logger) (string, error) {
	...
	_, err = jsoner.Parse(rawBytes, &params, jsoner.UseNumber)
	if err != nil {
		return "", err
	}
