...
func newApp() (*server.App, func(), error) {
	...
	sidSid := sid.NewSid()
	serviceService := service.NewService(repoRepository, logger, sidSid, apiConf)
	...
	sender := mail.NewSender(logger, apiConf)
	...
	app := server.NewServerHTTP(logger, apiConf, client, demoApi, userHandler, loginHandler, sysSettingHandler)
	return app, func() {
	}, nil
}
...
var repoGroup = wire.NewSet(repo.NewWDB, repo.NewRDB, repo.NewRedis, repo.NewRepository, sysRepoSet,
	userRepoSet,
)
